...
const DebitHistory = () => {
    ...
    function getStatusMessage(status: boolean) {
      if (status === true) {
        return <p className="inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium text-success bg-success">Paid</p>;
      } else {
      ...
      <>
        ...
        {userData.map((user, key) => (
          ...
          >
            ...
            <div className="flex items-center justify-center col-span-2">
            <p className="text-md text-black dark:text-white">{getStatusMessage(user.status)}</p>
          </div>
          </div>
        ))}
